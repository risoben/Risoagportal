...
export function LocationFormComplete({ mode = 'Create', locationId }: LocationFormCompleteProps) {
  ...
  const handleSaveDynamic = () => {
    ...
    updateBenefitLimit('mittagessen', 'monthly', String(monthly));
  ...
  const handleSaveFix = () => {
    ...
    updateBenefitLimit('mittagessen', 'monthly', String(num));
                    ...
                    >
                      ...
                      <div className="flex items-center">
                        {benefit.id === 'mittagessen' ? (
                          <div className="flex items-center gap-2">
                            <div className={`flex rounded border overflow-hidden transition ${loadingState || !activeBenefits.has(benefit.id) ? 'opacity-40 pointer-events-none border-[#E0E0E0]' : 'border-[#0F429F]'}`}>
                              <button
                              ...
                              >🔄 Auto</button>
                              <button
                                onClick={() => { setFixModalValue(benefitLimits['mittagessen']?.monthly ?? ''); setFixModalError(''); setShowFixModal(true); }}
                            ...
                            <span className={`text-[12px] ${essenBudgetType === 'dynamic' ? 'text-[#0F429F]' : 'text-[#333]'}`}>
                              {benefitLimits['mittagessen']?.monthly} €
                            </span>
                          </div>
                        ) : (
      ...
      {showDynamicModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full" style={{ borderRadius: '8px' }}>
            ...
            <p className="text-[13px] text-[#666666] mb-5">Tagessatz × Arbeitstage (passt sich jährlich an)</p>
            <div className="mb-3">
